...
namespace Clinics.UserControls
{
    public partial class visitpatient : UserControl
    {
        ...
        public static visitpatient visitpatient2;
        public visitpatient()
        {
            InitializeComponent();
            visitpatient2 = this;
        ...
        private void textBox_Name_pat_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    SqlCommand cmd23 = new SqlCommand("select phone_pat from Table_PAT where ID_pat=@ID_pat", con);
                    cmd23.Parameters.Add(new SqlParameter("@ID_pat", textBox_ID_pat.Text));
                    con.Open();
                    SqlDataReader Ra3 = cmd23.ExecuteReader();
                    if (Ra3.Read())
                    {

                        txtPohne.Text = Convert.ToString(Ra3["phone_pat"]);
                    }
                    Ra2.Close();
                    con.Close();
                    //-------------------------------------------------------------------------------------------------------------
